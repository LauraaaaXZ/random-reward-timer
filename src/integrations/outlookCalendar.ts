...
type GraphCalendar = { id: string; name?: string | null };
type GraphCalendarsPage = { value: GraphCalendar[]; '@odata.nextLink'?: string };
type GraphEvent = {
  id: string;
  subject?: string | null;
  start: GraphDateTime;
...
type GraphEventsPage = { value: GraphEvent[]; '@odata.nextLink'?: string };

function asUtcIso(dateTime: GraphDateTime) {
  if (/[zZ]|[+-]\d\d:\d\d$/.test(dateTime.dateTime)) return new Date(dateTime.dateTime).toISOString();
...
async function listCalendars(accessToken: string) {
  ...
  let url: string | undefined = `${GRAPH_ROOT}/me/calendars?$select=id,name&$top=100`;
  while (url) {
    const page = await graphJson<GraphCalendarsPage>(url, accessToken);
...
async function listCalendarEvents(
  ...
  const params = new URLSearchParams({
    ...
    endDateTime: endAt,
    '$select': 'id,subject,start,end,isCancelled,isAllDay,showAs,lastModifiedDateTime',
    '$orderby': 'start/dateTime',
    '$top': '200',
  });
  let url: string | undefined =
    `${GRAPH_ROOT}/me/calendars/${encodeURIComponent(calendar.id)}/calendarView?${params.toString()}`;
  const events: ExternalCalendarEvent[] = [];

  while (url) {
    const page = await graphJson<GraphEventsPage>(url, accessToken);
...
export const outlookCalendarProvider: CalendarProvider = {
  ...
  async fetchEvents({ accessToken, startAt, endAt }) {
    ...
    const perCalendar = await Promise.all(
      calendars.map(calendar => listCalendarEvents(calendar, accessToken, startAt, endAt)),
    );
