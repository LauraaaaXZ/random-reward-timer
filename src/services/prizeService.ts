...
async function settleReward(reward:PrizeReward|TenDrawChoice,source='prize_pool'){
 if(reward.type==='coin')await applyReward({source,coin:reward.amount??0,xp:0});
 else if(reward.type==='annual_leave_minutes')await grantAnnualLeaveMinutes(reward.amount??reward.minutes??0);
 else if(reward.type==='freedom_credit')await addFreedomCredits(reward.amount??reward.quantity??1);
