...
export type TaskCompletionSettlement={drawProgress:number;coin:number;xp:number};
...
export async function markTaskCompleted(taskId:string):Promise<TaskCompletionSettlement>{const tasks=await listTasks(),task=tasks.find(c=>c.id===taskId);if(!task)throw new Error('Task not found.');const today=localDateKey();if(task.dailyPoolMode&&task.dailyLastCompletedDate===today)return{drawProgress:0,coin:0,xp:0};const reward=taskCompletionReward(task.difficulty,Math.max(5,task.estimatedMinutes));const sourceKey=`task:${task.id}:${task.dailyPoolMode?today:'complete'}`;
  // Settle idempotent rewards before changing task state. If either write fails,
  // the task remains retryable; both repositories protect against duplicate credit.
  await addDrawProgress(reward.drawProgress,sourceKey);
  await applyReward({sessionId:sourceKey,source:'task_completion',coin:reward.coin,xp:0});
  if(task.dailyPoolMode)await markDailyTaskDone(taskId,today);else{await updateTask({...task,remainingMinutes:0,status:'completed',completedAt:new Date().toISOString()});await refreshTaskLocks()}
  return{drawProgress:reward.drawProgress,coin:reward.coin,xp:0}}
