...
export function CalendarScreen(){
 ...
 const load=useCallback(async()=>{
  ...
  for(const d of days){const k=dateKey(d),b=dayBounds(d),events=await listCalendarBlocks(b.start,b.end);
   events.forEach((e:CalendarBlock)=>out.push({id:`e:${e.id}`,date:k,start:clock(new Date(e.startAt)),end:clock(new Date(e.endAt)),title:e.title,kind:e.kind==='meal'?'meal':e.kind==='sleep'?'sleep':'event'}));
   routines.forEach(r=>out.push({id:`r:${r.id}:${k}`,date:k,start:r.windowStart,end:r.windowEnd,title:r.name,kind:'routine'}));
  }
  taskBlocks.forEach(b=>{const d=new Date(b.startAt),k=dateKey(d);out.push({id:`tb:${b.id}`,date:k,start:clock(d),end:clock(new Date(b.endAt)),title:b.taskName,kind:'task'})});
 ...
 <Text style={s.foot}>Outlook/device events are read-only and sync automatically. Default routines, meals, sleep and scheduled tasks are app-local; task deadlines appear as markers.</Text></ScrollView>
}
