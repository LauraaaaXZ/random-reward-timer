...
export async function listCalendarBlocks(startAt:string,endAt:string):Promise<CalendarBlock[]>{
 const db=await getDatabase();const[localRows,externalRows,meals,sleep]=await Promise.all([
 ...
 db.getAllAsync<ExternalCalendarRow>('SELECT provider,external_id,title,start_at,end_at FROM external_calendar_events WHERE end_at > ? AND start_at < ? ORDER BY start_at',startAt,endAt),listMealSchedules(),getSleepSchedule()]);
 const rangeStart=new Date(startAt),rangeEnd=new Date(endAt),mealBlocks:CalendarBlock[]=[],sleepBlocks:CalendarBlock[]=[];const cursor=new Date(rangeStart);cursor.setHours(0,0,0,0);cursor.setDate(cursor.getDate()-1);
 while(cursor<rangeEnd){const day=localDateKey(cursor);for(const meal of meals){const s=new Date(`${day}T${meal.startTime}:00`),e=new Date(s.getTime()+3600000);if(e>rangeStart&&s<rangeEnd)mealBlocks.push({id:`meal:${meal.id}:${day}`,title:meal.label,startAt:s.toISOString(),endAt:e.toISOString(),kind:'meal'})}if(sleep){const s=new Date(`${day}T${sleep.startTime}:00`),e=new Date(s.getTime()+sleep.durationMinutes*60000);if(e>rangeStart&&s<rangeEnd)sleepBlocks.push({id:`sleep:${sleep.id}:${day}`,title:'Sleep',startAt:s.toISOString(),endAt:e.toISOString(),kind:'sleep'})}cursor.setDate(cursor.getDate()+1)}
