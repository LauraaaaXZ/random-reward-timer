...
export async function getHolidayPassCount() {
  ...
  const row = await db.getFirstAsync<{ quantity: number }>(
    'SELECT quantity FROM inventory WHERE resource_key = ?',
    'holiday_pass',
  );
...
export async function addHolidayPass(quantity = 1) {
  ...
  await db.runAsync(
    `INSERT INTO inventory (resource_key, quantity, updated_at)
     VALUES ('holiday_pass', ?, ?)
...
export async function redeemHolidayPassWithCoin() {
  const db = await getDatabase();
  const cost = GAME_CONFIG.holidayPass.coinCost;
  const now = new Date().toISOString();
  ...
  await db.withTransactionAsync(async () => {
    ...
    await db.runAsync(
      'UPDATE wallet SET coin = coin - ?, updated_at = ? WHERE id = 1',
      cost,
      now,
    );
    await db.runAsync(
      `INSERT INTO inventory (resource_key, quantity, updated_at)
       VALUES ('holiday_pass', 1, ?)
       ON CONFLICT(resource_key) DO UPDATE SET quantity = quantity + 1, updated_at = excluded.updated_at`,
      now,
    );
    await db.runAsync(
      `INSERT INTO reward_events (id, session_id, source, coin_delta, xp_delta, multiplier, created_at)
       VALUES (?, NULL, 'holiday_pass_exchange', ?, 0, 1, ?)`,
      randomUUID(),
      -cost,
      now,
    );
...
export async function scheduleHoliday(localDate: string) {
  ...
  await db.withTransactionAsync(async () => {
    ...
    const pass = await db.getFirstAsync<{ quantity: number }>(
      'SELECT quantity FROM inventory WHERE resource_key = ?',
      'holiday_pass',
    );
    ...
    await db.runAsync(
      'UPDATE inventory SET quantity = quantity - 1, updated_at = ? WHERE resource_key = ?',
      now,
      'holiday_pass',
    );
    await db.runAsync(
      'INSERT INTO holiday_days (local_date, source, created_at) VALUES (?, ?, ?)',
      localDate,
      'pass',
      now,
    );
...
export async function grantHolidayFromDraw(localDate: string) {
  ...
  const now = new Date().toISOString();
  await db.runAsync(
    'INSERT OR REPLACE INTO holiday_days (local_date, source, created_at) VALUES (?, ?, ?)',
    localDate,
    'draw',
    now,
  );
...
export async function listUpcomingHolidays(fromLocalDate: string) {
  const db = await getDatabase();
  const rows = await db.getAllAsync<{ local_date: string; source: HolidayDay['source']; created_at: string }>(
